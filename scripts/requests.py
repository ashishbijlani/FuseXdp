...
def plot_chart(df):
	...
	df.plot(ax=ax, kind='bar', x='Request-Type', align="center",
...
def plot_fig():
	...
	for key, item in df.iterrows():
		...
		ax = af.plot(ax=axs[plot_num], kind='bar', title='', edgecolor="red", align="center", \
...
def add_to_dataframe_list(df_list, list_lines):
	...
	records = {}
	# filter lines
	for line in cleaned_list_lines:
		# marker
		if ':' in line:
			marker = ':'
		elif ',' in line:
			...
			return None
		for k,v in dict([line.split(marker)]).items():
			try:
	...
	if df['Request-Type'].any() != 'FUSE_BATCH_FORGET':
		df_list.append(df)
	return df_list
...
if __name__ == "__main__":
	...
	if len(sys.argv) == 3:
		...
		pattern="^$"
	delimeter_pattern = re.compile(pattern)
